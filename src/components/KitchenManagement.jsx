...
import { X, ChefHat, Trash2 } from 'lucide-react';
    ...
    <div className="flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden h-full">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
        ...
        {pending.map((item, idx) => (
          <button
            key={`${item.billId}-${item.orderItemId || item.menuItemId}-${item.batchOrder ?? idx}`}
            onClick={() => onComplete(item)}
            className="w-full text-left px-3 py-2 hover:bg-indigo-50 transition-colors group"
          >
        ...
        {completed.map((item, idx) => (
          <button
            key={`done-${item.billId}-${item.orderItemId || item.menuItemId}-${item.batchOrder ?? idx}`}
            onClick={() => onUndo(item)}
...
const KitchenManagement = ({ onClose, selectedDate }) => {
  ...
  const handleUndo = (item) =>
    undoCompleted(item.billId, item.orderItemId || item.menuItemId);
